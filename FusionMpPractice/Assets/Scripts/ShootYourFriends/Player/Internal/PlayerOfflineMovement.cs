...
namespace PlayerManagement
{
    public class PlayerOfflineMovement : MonoBehaviour
    {
        ...
        internal Vector3 m_moveVector;
        internal bool m_JumpButtonIsPressed;
        internal bool m_KneelButtonIsPressed;
        #endregion
        ...
        private void Awake()
        {
            ...
            m_playerOfflineController.m_eEAvatarMoveState = EAvatarMoveState.Walking;
        }

        private void OnDisable()
        {
            if (m_playerOfflineController.m_playerOfflineInput.gameObject.activeInHierarchy)
            {
        ...
        private void Start()
        {
            if (m_playerOfflineController.m_playerOfflineInput.gameObject.activeInHierarchy)
            {
                ...
                m_playerOfflineController.m_rePopPosition = m_rigidbodyTransform.position;
            }
        }
        ...
        private void Update()
        {
            ...
            if (m_rigidbodyTransform.position.y < m_playerOfflineController.m_fallLimit)
            {
                m_rigidbodyTransform.position = m_playerOfflineController.m_rePopPosition; //AreaFallOffReset
            }
        }
            ...
            if (!m_playerOfflineController.m_isDead)
            {
                ...
                switch (m_playerIsGrounded) //Calculate FallDamage.
                {
                    ...
                    case true:
                    {
                        ...
                        if (!m_JumpButtonIsPressed && !m_canJumpAgain)
                        {
                            m_canJumpAgain = true;
                        }
                        break;
        ...
        {
            if (m_coyoteTimeCounter >= 0 && m_JumpButtonIsPressed && m_canJumpAgain && m_playerIsGrounded)
            {
        ...
        {
            m_moveDirection = new Vector3(m_moveVector.x, 0.0f, m_moveVector.z);
            ...
            if (m_moveDirection != Vector3.zero)
            {
                ...
                m_targetRotation = Quaternion.RotateTowards(m_rigidbodyTransform.rotation, m_targetRotation, m_quaternionRotTime * Time.fixedDeltaTime);
        ...
        {
            m_moveDirection = new Vector3(0.0f, 0.0f, m_moveVector.z);
            m_moveDirection = m_rigidbodyTransform.TransformDirection(m_moveDirection);
            m_rigidbody.MovePosition(m_rigidbodyTransform.position + m_individualMaxSpeed * Time.fixedDeltaTime * m_moveDirection.normalized);

            m_quatDeltaRot =
                Quaternion.Euler(0.0f, m_moveVector.y * Time.fixedDeltaTime * (m_quaternionRotTime * m_aDRotYReduction), 0.0f);
        ...
        private void MoveRigidBodyMouseY()
        {
            ...
            m_quatDeltaRot =
                Quaternion.Euler(0.0f, m_moveVector.y * Time.fixedDeltaTime * (m_quaternionRotTime * m_mouseRotYReduction), 0.0f);
            m_rigidbody.MoveRotation(m_rigidbody.rotation * m_quatDeltaRot);
        }
        ...
        private void Crouching()
        {
            if (m_permitCrouchLerp)
            {
                ...
                switch (m_KneelButtonIsPressed) //fomor: m_kneelToCrouch.
                {
                    case false:
                    {
        ...
        private void MoveAcceleration()
        {
            switch (m_moveButtonIsPressed)
            {
                ...
                {
                    switch (m_KneelButtonIsPressed) //In case the character shall slow down from Walking or Running.
                    {
                        case false:
                        {
                            m_playerOfflineController.m_eEAvatarMoveState = EAvatarMoveState.Idle;
                        ...
                        case true:
                        {
                            m_playerOfflineController.m_eEAvatarMoveState = EAvatarMoveState.Crouching;
                    ...
                    switch (m_shiftIsPressed)   //Runspeed Switch
                    {
                        ...
                        case true:  //Shift IS pressed! <---
                        {
                            switch (m_KneelButtonIsPressed)
                            {
                                ...
                                {
                                    m_playerOfflineController.m_eEAvatarMoveState = EAvatarMoveState.Running;
        ...
        private void ApplyFallDamage(float _finalFallDistance)
        {
            ...
            m_playerOfflineController.m_playerOfflineHealth.TakeDamage(Mathf.Round(_finalFallDistance));
        }
        #endregion
        #endregion
