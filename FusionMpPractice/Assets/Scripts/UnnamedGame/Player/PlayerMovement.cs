...
public class PlayerMovement : MonoBehaviour
{
    ...
    private float m_stopMovement = 0.0f;
    ...
    private float m_startMoveLerp, m_endMoveLerp;           //Start / End
    private float m_lerpedMoveSpeed, m_lerpTimeCounter;     //increasing runTime Variables
    private bool m_playerIsGrounded, m_jumpIsPressed, m_shiftIsPressed = false;
    ...
    private void Awake()
    {
        ...
        m_lerpedMoveSpeed = m_playerWalkSpeed;
    }
    ...
    {
        float startLerpValue = m_startMoveLerp;
        switch (m_playerController.m_eCurrentMoveMode)
        {
            case EMoveModi.Walking:
                m_endMoveLerp = m_playerWalkSpeed;
                break;
            case EMoveModi.Running:
                m_endMoveLerp = m_playerRunSpeed;
                break;
            case EMoveModi.Crouching:
                m_endMoveLerp = m_playerCrouchSpeed;
                break;
        }
        if(startLerpValue < m_endMoveLerp)
        {
            //TODO: V3.Lerp.
        }
    }
    ...
    private void SetTargetSpeedMode(float _currentMoveSpeed, EMoveModi _targetMoveMode = EMoveModi.Walking)
    {
    ...
    private void CharacterDuck(InputAction.CallbackContext _callbackContext)
    {
        ...
        SetTargetSpeedMode(m_lerpedMoveSpeed, EMoveModi.Crouching);
    }
    ...
    private void StopDucking(InputAction.CallbackContext _callbackContext)
    {
        ...
        SetTargetSpeedMode(m_lerpedMoveSpeed);
    ...
    {
        SetTargetSpeedMode(m_lerpedMoveSpeed, EMoveModi.Running);
